...
ResultOrError<Ref<Device>> Device::Create(AdapterBase* adapter,
    ...
    Ref<Device> device = AcquireRef(new Device(adapter, descriptor, deviceToggles, overrideFunctions));
...
ResultOrError<VulkanDeviceKnobs> Device::CreateDevice(VkPhysicalDevice vkPhysicalDevice) {
    ...
    if (mOverrideFunctions && mOverrideFunctions->overrideVkCreateDevice != nullptr){
        DAWN_TRY(CheckVkSuccess(mOverrideFunctions->overrideVkCreateDevice(vkPhysicalDevice, &createInfo, nullptr, &mVkDevice, fn.GetInstanceProcAddr), "vkCreateDevice"));
    } else {
